        ...
        <div className="card border border-gray-800">
          ...
          {assets.length === 0 ? <div className="mt-5 rounded-xl border border-dashed border-gray-700 px-5 py-10 text-center text-sm text-gray-500">Upload a branded video, image, or audio file to start building reusable video bookends.</div> : <div className="mt-5 grid grid-cols-1 sm:grid-cols-2 gap-3 max-h-[610px] overflow-y-auto pr-1">{assets.map(asset => (
            <article key={asset.id} className="rounded-xl border border-gray-800 bg-gray-950/40 p-3">
              <div className="aspect-video rounded-lg overflow-hidden bg-black/60 flex items-center justify-center">{asset.media_type === 'image' && <img src={`${API_BASE}${asset.url}`} alt={asset.name} className="w-full h-full object-cover" />}{asset.media_type === 'video' && <video src={`${API_BASE}${asset.url}`} className="w-full h-full object-cover" controls muted playsInline />}{asset.media_type === 'audio' && <div className="w-full px-3"><p className="text-3xl text-unified-gold text-center mb-2">♪</p><audio src={`${API_BASE}${asset.url}`} controls className="w-full" /></div>}</div>
